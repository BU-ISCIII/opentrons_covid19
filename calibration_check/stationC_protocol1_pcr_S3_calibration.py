...
def run(ctx: protocol_api.ProtocolContext):
    ...
    if TRANSFER_SAMPLES:
        pick_up(p20, tips20)
        p20.move_to(sources[0].top())
        robot.pause(f"Is it at the top of the well?")
        p20.aspirate(VOLUME_ELUTION, sources[0].bottom(1))
        p20.move_to(sources[0].top())
        robot.pause(f"Did it aspirate correctly?")
        p20.move_to(dests[0].top())
        robot.pause(f"Is it at the top of the well?")
        p20.dispense(VOLUME_ELUTION, dests[0].bottom(2))
        p20.move_to(dests[0].top())
        robot.pause(f"Did it dispense all the liquid?")
        p20.move_to(sources[-1].top())
        robot.pause(f"Is it at the top of the well?")
        p20.aspirate(VOLUME_ELUTION, sources[-1].bottom(1))
        p20.move_to(sources[-1].top())
        robot.pause(f"Did it aspirate correctly?")
        ...
        robot.pause(f"Is it at the top of the well?")
        p20.dispense(VOLUME_ELUTION, dests[-2].bottom(2))
        p20.move_to(dests[-2].top())
        robot.pause(f"Did it dispense all the liquid?")
        drop(p20)
